...
4-1. 일정 수행 결과
   ...
   - 11월 26일 ~ 12월 2일 : stage2와 stage3 기본 맵 구성하였고 각 스테이지 간 상호작용을 구현하였다. 또한, 점프 사운드를 추가하고 게임 BGM을 설정하였다.
...
10. 구현하면서 어려운 부분, 하고 싶었지만 못 한 것들
    -
    - 캐릭터나 몬스터, 보스에 말풍선을 추가해서 스토리 진행을 하고 싶었으나 시간이 부족해서 패스하였다.
    - 앱을 스토어에 팔게 된다면 스토리 추가, 애니메이션 구체화, 더 다양한 함정을 추가해야 할 거 같다.
    - 마지막 보스 때 보스가 캐릭터를 향해 움직이게 하려고 했지만 구현하지 못했다.
  
12. 수업 진행 방식에 대한 제안
    -
    - 파이썬이라는 코드를 처음 배워 2D게임을 만들어보는 기회를 제공하는 수업이라 좋았다.
    - 텀 프로젝트를 진행하면서 깃을 더 활용하는 방법에 대해 배웠고 게임을 제작하는데 필요한 요소들을 알 수 있었다.
...
- 
- 고양이 마리오 BGM 유튜브 사이트 : https://www.youtube.com/playlist?list=PLOIlehl1iAoVCYB3dZ9Q6hybotCJC4Hnz
